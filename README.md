...
## ПОСИЛАННЯ НА ДІАГРАМУ КОМПОНЕНТІВ - [Діаграма компонентів](https://mermaid.live/edit#pako:eNqdVe9v2jAQ_VcsV5WolFBoaIBoqgRLtXXqrxWmSmv2wSQG3Bo7s50WVvV_3znOIC2om-YPkf24e_d890iecSoziiM8UySfo3GcCARLFxMHJPgjZ1QYdE5WVCXY_WyXw-8SfEsn1SHBP9zvVGSJ2GIaUfVIFRrkOWcpMUwK1LiE6s17fVBnHlyfAS080Sdi6BNZoUN0uswV1domr6vYtdnVJUthlOScKr2t264rlVG1iYJqJYI20Ksi5XXerTgsNBNW37mcsfSdorYHLKXritX5VTm7rhWgg4zkppR3IcVMSk0dDk_5yLJdKneqi4khaJCmpcBd0i5gClzX6zjkw0SdNEqhHoqZnh_sbsvucZ8uQbsgHJ2ziSKKUQ1zBN6CU10XUPJDzLoncDsrVSqbWUoYr3I6ShXLzcFfPHYmpopoo4rUFIrWy8TDu0Z1wXiIBoYTUHGwTba_b20gaGoNqh1W_Qd8P8Gfx-NrdEN_FlQb1PgyuroESb5_Yl3rgq1xff9GFoZa_I3XXMwbEOLj8dU6-o8pxMZvddxG0xwkU5Gu0Jm4d1rX6dC1nUnfNNU2qO6trRqQbNVXFyxDd-fUEcj4WlC1spHOOS7G7Us-kh3eKuZaEg9rzR6ZFWdiVk0SDnQjZMo4j_am_akHI5UPNNoLgqDa-08sM_PoKF96qeRSRRNO0oc6DYzZEUwm_0dQjd2RZNPsn0mwB-9TluFoSrimHl5QtSD2jJ8tf4LNnC7AnRFsOZvNjTXqC2TlRHyXcoEjcDDkKVnM5muWIs_gfRgzAmZfrFFljQBeKoTBUb_XLUlw9IyXODoOu82joB-2j4N-JwzDjodXOPLDZrfTa_W7naNWt9tutXsvHv5Vlm01-50g6AIUhpDS7gUephkzUl24z0T5tXj5Da578ic)
...
|   №    | Сценарій                 |  Метод   | Маршрут           | Опис Імплементації                                                                                                                                     |
| :----: | :----------------------- | :------: | :---------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1.** | **Створення замовлення** |  `POST`  | `/api/orders`     | Обчислюється `totalPrice` на основі `Menu.json`, присвоюється статус `"active"` та унікальний ID. Дані додаються до масиву `orders` (`data/order.js`). |
| **2.** | **Оновлення замовлення** |  `PUT`   | `/api/orders/:id` | Змінює поле `status`. При переході в стан `"completed"`, замовлення копіюється в масив історії (`data/history.json`).                                  |
| **3.** | **Видалення замовлення** | `DELETE` | `/api/orders/:id` | Видаляє замовлення з масиву `orders` **тільки** після того, як його статус змінено на `"completed"`.                                                   |

### 2\. Резервування та Аналітика

|   №    | Сценарій                | Метод  | Маршрут                         | Контролер                                                                                                                                               |
| :----: | :---------------------- | :----: | :------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **4.** | **Резервування столів** | `POST` | `/api/reservations`             | Імітує перевірку доступності столу на дату (конфлікт ±2 години) та зберігає бронювання у `data/reservation.js`.                                         |
| **5.** | **Агрегація даних**     | `GET`  | `/api/analytics/popular-dishes` | Імітує роботу MongoDB Aggregation Pipeline: підраховує загальну кількість замовлених страв (`menu_id`) з масиву `history` та сортує їх за популярністю. |

## 1. Навантажувальне тестування

Тестування проводилось при навантаженні 100-500 користувачів.
Було виявлено проблеми з продуктивністю (Event Loop Lag).

### Графік Event Loop Delay та CPU Usage

![Event Loop Graph](screenshots/doctor.png)
_Видно червону лінію затримки 400ms на початку, що свідчить про блокування потоку. Але потім знижається до стабільних 5-7ms_
_Процесор завантажений понад 100%, моментами і 700%, що підтверджує наявність важких обчислень._

---

## 2. Аналіз Бази Даних

Перевірка запиту `find({ status: 'active' })` показала відсутність індексів.

### Результат:

![DB Analysis](screenshots/tests.png)
_Статус `COLLSCAN` свідчить про повний перебір колекції._
